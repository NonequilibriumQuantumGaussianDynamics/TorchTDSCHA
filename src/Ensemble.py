...
from model_multi import get_AB
...
try:
    ...
    julia.Main.include(os.path.join(os.path.dirname(__file__), "fourier_gradient.jl"))
    __JULIA_EXT__ = True
except:
    try:
        import julia
        ...
        if supercell is not None:
            ...
            for i in range(3):
                if self.supercell[i] != dyn0.GetSupercell()[i]:
                    raise ValueError(
                        """Error, you specified a supercell of {},
...
""".format(
        ...
        for i in range(nat_sc):
            self.r_lat[i, :] = (
                self.supercell_structure.coords[i, :]
        ...
        # Exact averages
        self.exact_averages = exact_averages
        self.phi = phi
        self.chi = chi
        self.psi = psi
        self.reference = reference  # coords at which the derivatives are computed
    ...
    def __setattr__(self, name, value):
        ...
        """.format(
                    name, type(self).__name__, similar_objects
                )
        ...
        if new_units == UNITS_HARTREE:
            if self.units == UNITS_DEFAULT:
                ...
                raise NotImplementedError(
                    "Error, I do not know how to convert between {} and {}.".format(
    ...
    def init(self):
        ...
        if self.fourier_gradient:
            ...
            self.forces_qspace = julia.Main.vector_r2q(
                self.forces.reshape((self.N, 3 * nat_sc)), self.q_grid, self.itau, self.r_lat
            )

            # These should be in Ry/A to be consistent with the units
            self.sscha_forces_qspace = (
                -julia.Main.multiply_matrix_vector_fourier(
                    dynq,
            ...
            self.sscha_energies[:] = (
                -julia.Main.multiply_vector_vector_fourier(
                    self.forces_qspace / CC.Units.A_TO_BOHR,
                    self.u_disps_qspace * CC.Units.A_TO_BOHR,
                )
                * 0.5
            )  # The conversion is useless, keep it for clarity

            self.sscha_forces = julia.Main.vector_q2r(
                self.sscha_forces_qspace, self.q_grid, self.itau, self.r_lat
    ...
    def load(
        ...
        load_noncomputed_ensemble=False,
        timer=None,
    ):
        """
        LOAD THE ENSEMBLE
        ...
        for i in range(self.N):
            ...
            if (
                os.path.exists(
                    os.path.join(data_dir, "scf_population%d_%d.dat" % (population, i + 1))
                )
                and not load_displacements
            ):
                if timer:
                    timer.execute_timed_function(
                        structure.read_scf,
                        os.path.join(data_dir, "scf_population%d_%d.dat" % (population, i + 1)),
                        alat=self.dyn_0.alat,
                    )
                else:
                    structure.read_scf(
                        os.path.join(data_dir, "scf_population%d_%d.dat" % (population, i + 1)),
                ...
                if timer:
                    self.u_disps[i, :] = timer.execute_timed_function(
                        structure.get_displacement, super_structure
                    ).ravel()
                else:
                ...
                if timer:
                    disp = (
                        timer.execute_timed_function(
                            np.loadtxt,
                            os.path.join(data_dir, "u_population%d_%d.dat" % (population, i + 1)),
                        )
                        / A_TO_BOHR
                    )
                else:
            ...
            if os.path.exists(force_path):
                if timer:
                    self.forces[i, :, :] = (
                        timer.execute_timed_function(np.loadtxt, force_path) * A_TO_BOHR
                    )
                else:
...
""".format(
            ...
            # Load stress
            if os.path.exists(
                os.path.join(data_dir, "pressures_population%d_%d.dat" % (population, i + 1))
            ):
                if timer:
                    self.stresses[i, :, :] = timer.execute_timed_function(
                        np.loadtxt,
                        os.path.join(
        ...
        if verbose:
            print(
                "[LOAD ENSEMBLE]: time elapsed for the cycle over the configurations:",
        ...
        # Load the energy
        if timer:
            total_energies = timer.execute_timed_function(
                np.loadtxt,
                os.path.join(data_dir, "energies_supercell_population%d.dat" % (population)),
            )
        else:
            total_energies = np.loadtxt(
                os.path.join(data_dir, "energies_supercell_population%d.dat" % (population))
            )
...
{}""".format(
                    np.arange(self.N)[~self.force_computed]
                )
                print(ERROR_MSG)
                raise IOError(ERROR_MSG)
...
{}""".format(
                    np.arange(self.N)[~self.stress_computed]
                )
                print(ERROR_MSG)
                raise IOError(ERROR_MSG)
    ...
    def load_from_calculator_output(self, directory, out_ext=".pwo", timer=None):
        ...
        output_files = [
            "{}/{}".format(directory, x) for x in os.listdir(directory) if x.endswith(out_ext)
        ]
    ...
    def save(self, data_dir, population, use_alat=False):
        ...
        for i in range(self.N):
            ...
            if self.force_computed[i]:
                np.savetxt(
                    "%s/forces_population%d_%d.dat" % (data_dir, population, i + 1),
                    self.forces[i, :, :] / A_TO_BOHR,
                )
            ...
            if use_alat:
                struct.save_scf(
                    "%s/scf_population%d_%d.dat" % (data_dir, population, i + 1),
                    self.dyn_0.alat,
                    True,
                )
            else:
            ...
            u_disp = self.u_disps[i, :].reshape(
                (struct.N_atoms, 3)
            )  # struct.get_displacement(super_dyn.structure)
            np.savetxt(
                "%s/u_population%d_%d.dat" % (data_dir, population, i + 1), u_disp * A_TO_BOHR
            )

            # Save the stress tensors if any
            if self.has_stress and self.stress_computed[i]:
                np.savetxt(
                    "%s/pressures_population%d_%d.dat" % (data_dir, population, i + 1),
                    self.stresses[i, :, :],
                )
    ...
    def save_bin(self, data_dir, population_id=1):
        ...
        if Parallel.am_i_the_master():
            ...
            if np.all(len(list(x)) > 0 for x in self.all_properties):
                with open(
                    os.path.join(data_dir, "all_properties_pop%d.json" % population_id), "w"
                ) as fp:
    ...
    def save_extxyz(self, filename, append_mode=True):
        ...
        for i, s in enumerate(self.structures):
            ...
            stress = (
                -self.stresses[i, :, :] * Rydberg / Bohr**3
            ...
            calculator = ase.calculators.singlepoint.SinglePointCalculator(
                struct, energy=energy, forces=forces, stress=CC.Methods.transform_voigt(stress)
            )
    ...
    def save_raw(self, root_directory, type_dict=None):
        ...
        if Parallel.am_i_the_master():
            ...
            if not os.path.isdir(root_directory):
                raise IOError(
                    "Error, save_raw expects a directory, but '{}' is not a directory.".format(
                        root_directory
                    )
                )
            ...
            # Save the positions
            np.savetxt(
                os.path.join(root_directory, "coord.raw"), self.xats.reshape((self.N, 3 * nat))
            )

            # Save the box
            # Prepare an array with all the unit cells
            np.savetxt(
                os.path.join(root_directory, "box.raw"),
                np.array([s.unit_cell.ravel() for s in self.structures]),
            )

            # Save the forces
            np.savetxt(
                os.path.join(root_directory, "force.raw"),
            ...
            np.savetxt(
                os.path.join(root_directory, "virial.raw"),
    ...
    def load_bin(self, data_dir, population_id=1, avoid_loading_dyn=False, timer=None):
        ...
        if os.path.exists(all_prop_fname):
            with open(
                os.path.join(data_dir, "all_properties_pop%d.json" % population_id), "r"
            ) as fp:
                try:
                    props = json.load(fp)
                    reading = True
                ...
                if not reading:
                    warnings.warn(
                        "WARNING: found file {} but not able to load the properties keyword.".format(
                            all_prop_fname
                        )
                    )
    ...
    def init_from_structures(self, structures):
        ...
        if self.fourier_gradient:
            self.u_disps_qspace = julia.Main.vector_r2q(
                self.u_disps, self.q_grid, self.itau, self.r_lat
            )
            self.u_disps_original_qspace = julia.Main.vector_r2q(
                self.u_disps_original, self.q_grid, self.itau, self.r_lat
            )
            ...
            self.sscha_energies[:] = (
                julia.Main.multiply_vector_vector_fourier(
                    self.sscha_forces_qspace, self.u_disps_original_qspace * CC.Units.A_TO_BOHR
                )
                * 0.5
            )
    ...
    def init_q_opposite(self):
        ...
        if self.fourier_gradient:
            ...
            self.q_opposite_index = julia.Main.get_opposite_q(
                np.array(self.current_dyn.q_tot, dtype=np.float64), bg
            )
    ...
    def generate(
        ...
        if evenodd:
            ...
            )  # normal Sobol generator****Diegom_test****
            ...
            structures = self.dyn_0.ExtractRandomStructures(
                N,
                self.T0,
                project_on_vectors=project_on_modes,
                lock_low_w=self.ignore_small_w,
    ...
    def update_weights_fourier(self, new_dynamical_matrix, newT, timer=None):
        ...
        changed_dyn = np.max(
            [
                np.max(np.abs(self.current_dyn.dynmats[i] - new_dynamical_matrix.dynmats[i]))
        ...
        changed_struct = (
            np.max(
                np.abs(self.current_dyn.structure.coords - new_dynamical_matrix.structure.coords)
            )
            > 1e-10
        )
        ...
        if self.ignore_small_w:
            violating_sum_rule = np.sum(trans_mask.astype(int)) != np.sum(
                trans_original.astype(int)
            )
...
""".format(
                np.sum(trans_original.astype(int)), np.sum(trans_mask.astype(int))
            )
        ...
        Y_qspace_old = julia.Main.get_upsilon_fourier(
            self.w_q_0, self.pols_q_0, self.dyn_0.structure.get_masses_array(), np.float64(self.T0)
        )
        t2 = time.time()
        ...
        v_new = julia.Main.multiply_matrix_vector_fourier(Y_qspace_new, u_disp_fourier_new)
        v_old = julia.Main.multiply_matrix_vector_fourier(Y_qspace_old, u_disp_fourier_old)

        uYu_new = julia.Main.multiply_vector_vector_fourier(
            u_disp_fourier_new * CC.Units.A_TO_BOHR, v_new * CC.Units.A_TO_BOHR
        )
        uYu_old = julia.Main.multiply_vector_vector_fourier(
            u_disp_fourier_old * CC.Units.A_TO_BOHR, v_old * CC.Units.A_TO_BOHR
        )
        t3 = time.time()
        if timer:
    ...
    def update_weights(self, new_dynamical_matrix, newT, update_q=False, timer=None):
        ...
        self.u_disps[:, :] = self.xats.reshape((self.N, 3 * Nat_sc)) - np.tile(
            super_structure.coords.ravel(), (self.N, 1)
        )
        old_disps[:, :] = self.xats.reshape((self.N, 3 * Nat_sc)) - np.tile(
            super_struct0.coords.ravel(), (self.N, 1)
        )
        ...
        if timer:
            self.sscha_energies[:], self.sscha_forces[:, :, :] = timer.execute_timed_function(
                new_dynamical_matrix.get_energy_forces,
                None,
                displacement=self.u_disps,
                w_pols=(w_new, pols),
            )
        else:
            self.sscha_energies[:], self.sscha_forces[:, :, :] = (
                new_dynamical_matrix.get_energy_forces(
...
""".format(
    ...
    def get_average_energy(self, subtract_sscha=False, return_error=False):
        ...
        if not self.exact_averages:
            ...
            R = coords - ref_coords
            R = np.reshape(R, nmod)
            R = np.einsum("i,i->i", R, np.sqrt(masses)) * 1.88973

            om, eigv = self.current_dyn.DiagonalizeSupercell()
            A, B = get_AB(om, eigv, self.current_T)

            energy = av_V(R, A, phi, chi, psi)

            if subtract_sscha:
                Phi = np.einsum("s,is,js->ij", om**2, eigv, eigv)
                energy_harm = 1 / 2 * np.einsum("ij,ij", Phi, A)
                energy = energy - energy_harm
        ...
        if not self.exact_averages:
            ...
            if in_unit_cell and not np.prod(self.supercell) == 1:
                ...
                for i in range(nat):
                    ...
                    new_forces[:, i, :] = np.sum(eforces[:, itau == i, :], axis=1) / np.prod(
                        self.supercell
                    )
                    # new_forces[:, i, :] =
            ...
            force = exact_force(R, A, phi, chi, psi)
            force = force * np.sqrt(masses) * 1.88973  # Ry / A
            force = np.reshape(force, (nat, 3))

            if get_error:
                return force, np.ones(np.shape(force))  # 1 meV/A * symmetries
    ...
    def get_free_energy_interpolating(
        self,
        target_supercell,
        support_dyn_coarse=None,
        support_dyn_fine=None,
        error_on_imaginary_frequency=True,
        return_error=False,
    ):
        """
        ...
        if support_dyn_fine is not None:
            new_dyn = self.current_dyn.Interpolate(
            ...
            new_dyn = self.current_dyn.Interpolate(
                self.current_dyn.GetSupercell(), target_supercell
            )
        ...
        harm_fe = new_dyn.GetHarmonicFreeEnergy(self.current_T, not error_on_imaginary_frequency)
    ...
    def get_fc_from_self_consistency(self, subtract_sscha=False, return_error=False):
        ...
        if subtract_sscha:
            f_vector = (self.forces - self.sscha_forces).reshape(
        ...
        new_phi = (new_phi + np.transpose(new_phi)) * 0.5
        ...
        if return_error:
            ...
            delta_new_phi = (delta_new_phi + np.transpose(delta_new_phi)) * 0.5
...
"""
        ...
        v_tilde = julia.Main.multiply_matrix_vector_fourier(
            Y_qspace, self.u_disps_qspace * CC.Units.A_TO_BOHR
        )
        ...
        def get_gradient_worker(cfgs, timer=None):
            ...
            phi_grad, phi_grad2 = julia.Main.get_gradient_fourier(
                new_vtilde, new_delta_forces, new_rho, self.q_opposite_index
            )
        ...
        if timer is not None:
            gradient_and_error = timer.execute_timed_function(
                CC.Settings.GoParallel, get_gradient_worker, configs_ranges, "+"
            )
        else:
            gradient_and_error = CC.Settings.GoParallel(get_gradient_worker, configs_ranges, "+")
    ...
    def get_preconditioned_gradient(
        self,
        subtract_sscha=True,
        return_error=False,
        use_ups_supercell=True,
        preconditioned=1,
        fast_grad=False,
        verbose=True,
        timer=None,
        ...
        if fast_grad or not preconditioned:
            if timer:
                grad, grad_err = timer.execute_timed_function(
                    SCHAModules.get_gradient_supercell,
                    self.rho,
                    u_disp,
                    eforces,
                    w,
                ...
                grad, grad_err = SCHAModules.get_gradient_supercell(
                    self.rho,
                    u_disp,
                    eforces,
                    w,
            ...
            if not self.exact_averages:
                if timer:
                    grad, grad_err = timer.execute_timed_function(
                        SCHAModules.get_gradient_supercell_new,
                        self.rho,
                        u_disp,
                        eforces,
                        w,
                    ...
                    grad, grad_err = SCHAModules.get_gradient_supercell_new(
                        self.rho,
                        u_disp,
                        eforces,
                        w,
                ...
                Phi = np.einsum("s,is,js->ij", om**2, eigv, eigv)
                grad = kappa(R, A, phi, chi, psi) - Phi
                grad = np.einsum("ij,i,j->ij", grad, np.sqrt(masses), np.sqrt(masses))
                grad = -grad

                grad_err = np.ones(np.shape(grad))  # error = 1 default
        ...
        if np.prod(self.dyn_0.GetSupercell()) > 1:
            ...
            if return_error:
                ...
                if (
                    hasattr(CC.Phonons, "GetDynQFromFCSupercell_parallel")
                    and CC.Settings.GetNProc() > 1
                ):
                    if timer:
                        q_grad, q_grad_err = timer.execute_timed_function(
                            CC.Phonons.GetDynQFromFCSupercell_parallel,
                            grad,
                            np.array(self.current_dyn.q_tot),
                            self.current_dyn.structure,
                            super_struct,
                            fc2=grad_err,
                        )
                    else:
                        q_grad, q_grad_err = CC.Phonons.GetDynQFromFCSupercell_parallel(
                            grad,
                            np.array(self.current_dyn.q_tot),
                            self.current_dyn.structure,
                            super_struct,
                    ...
                    if timer:
                        q_grad, q_grad_err = timer.execute_timed_function(
                            CC.Phonons.GetDynQFromFCSupercell,
                            grad,
                            np.array(self.current_dyn.q_tot),
                            self.current_dyn.structure,
                            super_struct,
                        ...
                        q_grad, q_grad_err = CC.Phonons.GetDynQFromFCSupercell(
                            grad,
                            np.array(self.current_dyn.q_tot),
                            self.current_dyn.structure,
                            super_struct,
                ...
                if hasattr(CC.Phonons, "GetDynQFromFCSupercell_parallel"):
                    if timer:
                        q_grad = timer.execute_timed_function(
                            CC.Phonons.GetDynQFromFCSupercell_parallel,
                            grad,
                            np.array(self.current_dyn.q_tot),
                            self.current_dyn.structure,
                        ...
                        q_grad = CC.Phonons.GetDynQFromFCSupercell_parallel(
                            grad,
                            np.array(self.current_dyn.q_tot),
                            self.current_dyn.structure,
                            super_struct,
                    ...
                    if timer:
                        ...
                        q_grad = CC.Phonons.GetDynQFromFCSupercell(
                            grad,
                            np.array(self.current_dyn.q_tot),
                            self.current_dyn.structure,
                            super_struct,
    ...
    def get_stress_tensor(self, offset_stress=None, use_spglib=False):
        ...
        stress, err_stress = SCHAModules.get_stress_tensor(
            volume,
            self.forces / __A_TO_BOHR__,
            u_disps * __A_TO_BOHR__,
            abinit_stress,
            wr,
            er,
            self.current_T,
    ...
    def get_free_energy_hessian(
        ...
        if not self.exact_averages:
            if timer:
                d3 = timer.execute_timed_function(
                    SCHAModules.get_v3,
                    a,
                    new_pol,
                    trans,
                    ...
                    self.rho,
                    log_err,
                    override_name="SCHAModules.get_v3",
                )
            else:
                d3 = SCHAModules.get_v3(a, new_pol, trans, amass, ityp, f, u, self.rho, log_err)
        else:
            chi = self.chi
            psi = self.psi
            ...
            coords = structure.coords
            R = (
                coords - ref_coords * 1.88973
            )  # coords are already in Ry, due to the conversion at the beginning of the routine
            R = np.reshape(R, nmod)
        ...
        if include_v4:
            ...
            t1 = time.time()
            if timer:
                d4 = timer.execute_timed_function(
                    SCHAModules.get_v4,
                    a,
                    new_pol,
                    trans,
                    ...
                    self.rho,
                    log_err,
                    override_name="SCHAModules.get_v4",
                )
            else:
                d4 = SCHAModules.get_v4(a, new_pol, trans, amass, ityp, f, u, self.rho, log_err)
            t2 = time.time()
            ...
            if timer:
                phi_sc_odd = timer.execute_timed_function(
                    SCHAModules.get_odd_straight_with_v4,
                    a,
                    w,
                    new_pol,
                    trans,
                    ...
                    d3,
                    d4,
                    override_name="SCHAModules.get_odd_straight_with_v4",
                )
            else:
                phi_sc_odd = SCHAModules.get_odd_straight_with_v4(
                    a, w, new_pol, trans, amass, ityp, self.current_T, d3, d4
                )
            if verbose:
            ...
            if timer:
                ...
                phi_sc_odd = SCHAModules.get_odd_straight(
                    a, w, new_pol, trans, amass, ityp, self.current_T, d3
                )
        ...
        if timer:
            dynq_odd = timer.execute_timed_function(
                CC.Phonons.GetDynQFromFCSupercell,
                phi_sc_odd,
                np.array(self.current_dyn.q_tot),
                self.current_dyn.structure,
            ...
            dynq_odd = CC.Phonons.GetDynQFromFCSupercell(
                phi_sc_odd,
                np.array(self.current_dyn.q_tot),
                self.current_dyn.structure,
                super_structure,
    ...
    def split(self, split_mask):
        ...
        if self.exact_averages:
            ens.phi = self.phi
            ens.chi = self.chi
            ens.psi = self.psi
            ens.reference = self.reference
            ens.exact_averages = self.exact_averages

        ens.all_properties = [
            self.all_properties[x] for x in np.arange(len(split_mask))[split_mask]
        ]
    ...
    def get_energy_forces(
        ...
        if __MPI__:
            ...
            if size > 1:
                ...
                nat3 = comm.bcast(
                    self.current_dyn.structure.N_atoms * 3 * np.prod(self.supercell), root=0
                )
        ...
        for i in range(start, stop):
            ...
            if Parallel.am_i_the_master() and verbose:
                print(
                    "Computing configuration %d out of %d (nat = %d)"
        ...
        self.forces[:, :, :] = np.reshape(
            total_forces,
            (N_rand, self.current_dyn.structure.N_atoms * np.prod(self.supercell), 3),
            order="C",
        )
...
def _wrapper_julia_matrix_vector_fourier(*args, **kwargs):
    ...
    return julia.Main.multiply_matrix_vector_fourier(*args, **kwargs)
...
def _wrapper_julia_vector_vector_fourier(*args, **kwargs):
    ...
    return julia.Main.multiply_vector_vector_fourier(*args, **kwargs)
