...
def torch_ext_for(t, field):
    ...
    # Convert everything to tensors (preserve dtype/device from Zeff)
    device, dtype = Zeff.device, Zeff.dtype
    edir = torch.as_tensor(edir, device=device, dtype=dtype)
    ...
    # Compute effective field and frequency
    Eeff = Eamp * 2.7502067e-7 * 2.0 / (1.0 + torch.sqrt(eps))
    freq = om_L / 2.0670687e4

    nmod = Zeff.shape[0]
    nat = nmod // 3

    # Compute the projected forces
    # Each atomic block (3×M) dotted with edir → (M,)
    root2 = math.sqrt(2.0)
    force_blocks = []
    for i in range(nat):
        blk = Zeff[3*i:3*i+3, :]             # (3, M)
        proj = blk.transpose(0, 1) @ edir     # (M,)
    ...
    if case == 'sine':
        ...
        t0_ = t0 / conv
        sig_ = sig / conv
        return -force * torch.cos(two_pi * freq * (t - t0_)) * torch.exp(-0.5 * (t - t0_)**2 / sig_**2)

    elif case == 'gaussian2':
        t0_ = t0 / conv
        sig_ = 1.0 / (math.sqrt(2.0) * math.pi * freq)
        return -force * (1.0 - (t - t0_)**2 / sig_**2) * torch.exp(-0.5 * (t - t0_)**2 / sig_**2)

    elif case == 'sinc':
        # torch.sinc(x) = sin(pi*x)/(pi*x) same as np.sinc
